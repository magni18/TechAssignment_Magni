...
function MainPage() {
  ...
  const [simulationState, setSimulationState] =
    useState<PossibleSimulaitonStates>(PossibleSimulaitonStates.OFF);
  const [currentSimulationVisualScore, setCurrentSimulationVisualScore] =
    useState<Team[][]>([]);

  const currentSimulationScore = useRef<Team[]>(
    structuredClone(Teams.teams) as Team[]
  );
  ...
  const onStartSimulation = () => {
    ...
    const score = handleRandomizationOfTeamScores(
      TotalScoreAllowed,
      Teams.teams.length
    );
  ...
  const handleUpdateCurrentScore = () => {
    const teamsWithNotAllScores = currentSimulationScore.current.filter(
      (current) => {
        for (const preset of preSetScore) {
          if (
            preset.name === current.name &&
            current.currentScore < preset.currentScore
          ) {
            return true;
          }
        }
    ...
    const randomIndex = Math.round(
      Math.random() * (teamsWithNotAllScores.length - 1)
    );
  ...
  const handleSetUpMatches = (score: Team[]): Team[][] => {
    ...
    try {
      ...
      if (tempList.length > 0) returnList.push(tempList);
    }
